    ...
    $.ajax({
        ...
        $.ajax({
            ...
            if (!data.erro && dados.length > 0) {
                ...
                var th3 = $('<th>').html('Marca');
                var th4 = $('<th>').html('Placa');
                var tr = $('<tr>').append(th1, th2, th3, th4);
                ...
                dados.forEach(function (element) {
                    ...
                    var descMarca = '';
                    marcas.forEach(function(mark){
                        if(mark.id == element.marca) {
                            descMarca = mark.descricao;
                        }
                    });
                    var td3 = $('<td onclick="alterar('+element.id+')">').html(descMarca).css('cursor','pointer');
                    var td4 = $('<td onclick="alterar('+element.id+')">').html(element.placa).css('cursor','pointer');
...
function alterar(id) {
    if (id != null) {
        window.location.href = 'form.veiculo.php?id=' + id;
    } else {
        var checks = document.getElementsByClassName('check');
        var checados = [];
        for (var check of checks) {
        ...
        if (checados.length > 1) {
            alert("Selecione apenas 1 veículo para alterar!");
        } else if (checados.length < 1) {
            alert("Selecione 1 veículo para alterar!");
        } else {
...
function excluir() {
    ...
    if (checados.length == 0) {
        alert("Selecione pelo menos 1 veículo para excluir!");
    } else {
